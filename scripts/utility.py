...
def family_repl(m, cat_object_dict: Dict[str, "Cat"], raise_exception=False):
    """Helper function for process_text. If raise_exception is
    False, any error in formatting will not raise an
    exception, and will use a simple replacement "error" """

    # Protection in insert & the pronoun tags
    if m.group(0).upper().startswith(("{PRONOUN", "{VERB", "{INSERT}")):
        return m.group(0)

    inner_details = m.group(1).split("/")

    if inner_details[2] in [
        "grandparent",
        "parent",
        "parents_sibling",
        "sibling",
        ...
        # this is an "unknown link", so pull the neutral intermediary if required
        relationship = inner_details[2]
        intermediary = None
    elif (
        inner_details[1] in cat_object_dict.keys()
        and inner_details[2] in cat_object_dict.keys()
    ):
        # this is a known link, so try getting the relationship
        relationship = cat_object_dict[inner_details[1]].inheritance.get_relationship(
            cat_object_dict[inner_details[2]].ID
        )
        intermediary = cat_object_dict[inner_details[2]].ID
    else:
        if raise_exception:
            raise KeyError("Malformed familial relation")
        return "error3_familial_malformed"

    try:
        value = cat_object_dict[inner_details[1]].get_familial_term(
        ...
        logger.exception("Failed to find familial relation: " + m.group(1))
        print("Failed to find familial relation:", m.group(1))
        return "error4_familial_other"
...
def process_text(text, cat_dict, cat_object_dict=None, raise_exception=False):
    """Add the correct name, familial nouns and pronouns into a string."""

    cat_dict = {} if cat_dict is None else cat_dict
    cat_object_dict = {} if cat_object_dict is None else cat_object_dict

    adjust_text = re.sub(
        r"\{(.*?)}", lambda x: family_repl(x, cat_object_dict, raise_exception), text
    )
...
def scale(rect):
    rect[0] = round(rect[0] / 1600 * screen_x)
    rect[1] = round(rect[1] / 1400 * screen_y)
