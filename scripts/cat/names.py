...
class Name:
        ...
        if self.suffix and not load_existing_name:
            ...
            while (
                ...
                or (
                    ...
                    and not str(self.prefix) == ""
                )
                or (
                    self.suffix.lower() in self.prefix.lower()
                    and not str(self.suffix) == ""
                )
            ):
    ...
    @property
    def screenreader_name(self):
        """
        Returns the prefix and suffix with a space between them so that screenreaders pronounce it correctly more often
        (e.g. Stormheart becomes Storm heart, Rowdykit becomes Rowdy kit, etc.
        :return: screenreader friendly name
        """
        return self.prefix + str(self).replace(self.prefix, " ")
