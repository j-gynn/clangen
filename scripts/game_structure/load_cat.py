...
def csv_load(all_cats):
    ...
    if len(cat_data) > 0:
        ...
        for inter_cat in all_cats.values():
            ...
            inter_cat.mentor = registry.all_cats.get(inter_cat.mentor)
            apps = []
            former_apps = []
            for app_id in inter_cat.apprentice:
                app = registry.all_cats.get(app_id)
            ...
            for f_app_id in inter_cat.former_apprentices:
                f_app = registry.all_cats.get(f_app_id)
...
def save_check():
    ...
    for cat in registry.all_cats:
        cat_ob = registry.all_cats[cat]

        # Not-mutural mate relations
        # if cat_ob.mate:
        #    _temp_ob = registry.all_cats.get(cat_ob.mate)
        #    if _temp_ob:
...
def version_convert(version_info):
    ...
    if version < 1:
        ...
        for c in registry.all_cats.values():
