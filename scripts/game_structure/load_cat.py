...
def pronoun_load():
    pronoun_data = None
    clanname = game.switches["clan_list"][0]
    pronoun_json = f"{get_save_dir()}/{clanname}/pronouns.json"
    try:
        with open(pronoun_json, "r") as read_file:
            cat_data = ujson.loads(read_file.read())
    except FileNotFoundError:
        with open(pronoun_json, "w") as write_file:
            pass
    except PermissionError as e:
        game.switches["error_message"] = f"Can\t open {pronoun_json}!"
        game.switches["traceback"] = e
        raise
    except ujson.JSONDecodeError as e:
        game.switches["error_message"] = f"{pronoun_json} is malformed!"
        game.switches["traceback"] = e
        raise
